  ...
  return (
    <div className="space-y-4 mt-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-semibold text-white">Etiquetas (Tags)</h2>
          <p className="text-sm text-slate-400">Organize seus contatos com etiquetas coloridas personalizadas.</p>
        </div>
        <Button
          onClick={() => {
            setNewTagName('');
      ...
      {tags.length === 0 ? (
        <Card className="bg-slate-900 border-slate-700 ring-0 ring-transparent">
          <CardContent className="flex flex-col items-center justify-center py-12 text-center">
            <p className="text-slate-400 text-sm">Nenhuma etiqueta cadastrada ainda.</p>
            <p className="text-slate-500 text-xs mt-1">Crie etiquetas para categorizar seus contatos.</p>
          </CardContent>
        </Card>
      ) : (
        <Card className="bg-slate-900 border-slate-700 ring-0 ring-transparent">
        ...
        <DialogContent className="bg-slate-900 border-slate-700 sm:max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription className="text-slate-400">
              Crie uma nova etiqueta definindo o nome e a cor.
            </DialogDescription>
          </DialogHeader>

          <div className="space-y-4 py-2">
            <div className="space-y-2">
              <Label className="text-slate-300">Nome da Etiqueta</Label>
              <Input
      ...
      <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <DialogContent className="bg-slate-900 border-slate-700 sm:max-w-sm">
          <DialogHeader>
            <DialogTitle className="text-white">Excluir Etiqueta</DialogTitle>
            <DialogDescription className="text-slate-400">
              Tem certeza que deseja excluir a etiqueta &quot;{tagToDelete?.name}&quot;? Isso irá removê-la de todos os contatos vinculados. Esta ação não pode ser desfeita.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter className="bg-slate-900 border-slate-700">
            <Button
