...
export function PasswordForm() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!profile?.email) {
      toast.error('Não é possível alterar a senha sem um e-mail válido');
      return;
    }
    if (next.length < MIN_PASSWORD) {
      setConfirmError(`A senha deve ter no mínimo ${MIN_PASSWORD} caracteres`);
      return;
    }
    if (next !== confirm) {
    ...
    <Card className="bg-slate-900/40 border-slate-800">
      <CardHeader>
        ...
        <CardDescription className="text-slate-400">
          Use no mínimo {MIN_PASSWORD} caracteres. Você continuará conectado neste dispositivo após a alteração.
        </CardDescription>
      </CardHeader>
